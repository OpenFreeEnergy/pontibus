...
def default_settings():
    return ASFEProtocol.default_settings()
...
def test_validate_lambda_schedule_naked_charge(val, default_settings):
    ...
    with pytest.raises(ValueError, match=errmsg):
        ASFEProtocol._validate_lambda_schedule(
...
def test_validate_alchem_nonsmc(benzene_modifications):
    ...
    errmsg = "SmallMoleculeComponents are supported by this protocol"
    with pytest.raises(ValueError, match=errmsg):
        ASFEProtocol._validate_endstates(stateA, stateB)


def test_charged_alchem_comp(benzene_modifications_net_charge):
    stateA = ChemicalSystem(
        {
            "solute": benzene_modifications_net_charge["benzoic_acid"],
            "solvent": SolventComponent(),
        }
    )
    ...
    assert benzene_modifications_net_charge["benzoic_acid"].total_charge == -1
...
def test_extends_error(default_settings, stateA, stateB):
    ...
    protocol = ASFEProtocol(settings=default_settings)
